...
ssh -t ${SERVER_USER}@${DESTINATION_IP} -p $SERVER_PORT -i $IDENTITY_FILE >/dev/null <<EOF
    echo "$SERVER_USER_PASSWORD" | sudo -S mkdir -p $DESTINATION_PATH
    echo "$SERVER_USER_PASSWORD" | sudo mv -f $TEMP_PATH/* $DESTINATION_PATH
    echo "$SERVER_USER_PASSWORD" | sudo chown -R $SERVER_USER:$SERVER_USER $DESTINATION_PATH
    rm -rf $TEMP_PATH
    exit
EOF
